...
public class EtcdRegistry implements IRegistry{
    ...
    // 向ETCD中注册服务
    public void register(String serviceName, int port, int balanceWeight) throws Exception {
        ...
        ByteSequence val = ByteSequence.fromString(String.valueOf(balanceWeight));     // 目前只需要创建这个key,对应的value暂不使用,先留空
    ...
    public List<Endpoint> find(String serviceName) throws Exception {
        ...
        for (com.coreos.jetcd.data.KeyValue kv : response.getKvs()){
            ...
            String balanceWeight = kv.getValue().toStringUtf8();
            endpoints.add(new Endpoint(host,port, balanceWeight));
        }
        return endpoints;
    }
}
